...
def main():
    ...
    model = MixtureOfExperts(trained_experts, gating_network).to(config.device)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model.parameters(), lr=config.learning_rate)
    for epoch in range(config.num_epochs):
        for data in train_dataloader:
            # Forward pass
            outputs = get_expert_output(model, data)
    ...
    with torch.no_grad():
        for data in test_dataloader:
            predictions = get_expert_output(model, data)
            loss = criterion(predictions, data.y_relaxed.unsqueeze(-1))
